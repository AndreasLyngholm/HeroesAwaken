    ...
    <section id="main-content">
        ...
        <div class="row">
            <div class="small-16 columns">
                <h1><i class="fa fa-plus-circle"></i> Add Role</h1>
        ...
        <div class="row team">
            <div class="large-16 columns">
                <form method="POST" action="{{ route('admin.user.roles.doAdd') }}">
                    {{ csrf_field() }}
                    <div class="row">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @foreach($permission as $p)
                                                <input id="{{ camel_case($access . $p['access']) }}" type="checkbox" name="permissions[{{ strtolower($access . '.' . $p['access']) }}]"><label for="{{ camel_case($access . $p['access']) }}"><span data-tooltip aria-haspopup="true" class="has-tip" title="{{ $p['description'] }}">{{ preg_replace('/(?<!\ )[A-Z]/', ' $0', $p['access']) }}</span></label>
                                            @endforeach
                                        </td>
                                    </tr>
                                @endforeach
