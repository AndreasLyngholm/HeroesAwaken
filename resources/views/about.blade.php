@extends('partials.layout')

@section('content')

    <section id="main-content" style="margin-top: 50px;">

        <div class="row">
            <div class="small-16 columns">
                <h1>About Us</h1>
                <div class="big-sep"></div>
            </div>
        </div>

        <div class="row">
            <div class="small-16 columns">
                <p>
                    The heroes awaken team only has one goal, and that is to revive the game EA once made and closed their servers for. Heroes Awakens is a non profit community with open ideas for new suggestions and even new devs or staff members. If you feel like this is any kind of role for you, go to our contact us page and talk to someone there.
                </p>

            </div>
        </div>

        <div class="row team">
            <div class="small-16 columns">
                <div class="row small-up-1 medium-up-2 large-up-3">

                    <!--team-member-->
                    <div class="column team-member">
                        <div class="photo">
                            <img src="//placehold.it/215x280/fff/c5db3b" class="" alt="">
                        </div>
                        <h5>Makahost</h5>
                        <p>Founder, Developer &amp; Go-To Guy</p>
                    </div>
                    <!--//team-member-->

                    <!--team-member-->
                    <div class="column team-member">
                        <div class="photo">
                            <img src="//placehold.it/215x280/fff/c5db3b" class="" alt="">
                        </div>
                        <h5>Absent</h5>
                        <p>Staff</p>
                        
                    </div>
                    <!--//team-member-->

                    <!--team-member-->
                    <div class="column team-member">
                        <div class="photo">
                            <img src="//placehold.it/215x280/fff/c5db3b" class="" alt="">
                        </div>
                        <h5>Doge</h5>
                        <p>Staff, Doge</p>
                       
                    </div>
                    <!--//team-member-->

                    <!--team-member-->
                    <div class="column team-member">
                        <div class="photo">
                            <img src="//placehold.it/215x280/fff/c5db3b" class="" alt="">
                        </div>
                        <h5>Ray</h5>
                        <p>Staff</p>
 
                       
                    </div>
                    <!--//team-member-->

                    <!--team-member-->
                    <div class="column team-member">
                        <div class="photo">
                            <img src="//placehold.it/215x280/fff/c5db3b" class="" alt="">
                        </div>
                        <h5>Synaxis</h5>
                        <p>Staff</p>
                      
                    </div>
                    <!--//team-member-->

                    <!--team-member-->
                    <div class="column team-member">
                        <div class="photo">
                            <img src="//placehold.it/215x280/fff/c5db3b" class="" alt="">
                        </div>
                        <h5>Warp Productions</h5>
                        <p>Staff</p>
                    </div>
                    <!--//team-member-->
                    
                     <!--team-member-->
                    <div class="column team-member">
                        <div class="photo">
                            <img src="//placehold.it/215x280/fff/c5db3b" class="" alt="">
                        </div>
                        <h5>Lyngholm</h5>
                        <p>Web developer</p>
                    </div>

                          <!--team-member-->
                    <div class="column team-member">
                        <div class="photo">
                            <img src="//gyazo.com/74f4fb5b1fe608e9b6a38cf414f49267" class="" alt="">
                        </div>
                        <h5>Wazz</h5>
                        <p>Web developer</p>
                    </div>
                        
                          <!--team-member-->
                    <div class="column team-member">
                        <div class="photo">
                            <img src="//placehold.it/215x280/fff/c5db3b" class="" alt="">
                        </div>
                        <h5>Vahdek</h5>
                        <p>Web developer</p>
                    </div>
                        
                          <!--team-member-->
                    <div class="column team-member">
                        <div class="photo">
                            <img src="//placehold.it/215x280/fff/c5db3b" class="" alt="">
                        </div>
                        <h5>National_Mando</h5>
                        <p>Web developer</p>
                    </div>
                        
                          <!--team-member-->
                    <div class="column team-member">
                        <div class="photo">
                            <img src="//placehold.it/215x280/fff/c5db3b" class="" alt="">
                        </div>
                        <h5>BloodRecon</h5>
                        <p>Web developer</p>
                      
                    </div>

                </div>
            </div>
        </div>

    </section>

@endsection
